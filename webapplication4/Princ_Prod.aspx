<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Princ_Prod.aspx.cs" Inherits="WebApplication4.Princ_Prod" %>

<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <div class="products">
               <asp:SqlDataSource ID="dsprodutos0" runat="server" ConnectionString="<%$ ConnectionStrings:BD_SGBConnectionString %>" SelectCommand="SELECT TOP (10) vw_Princ_Prod.Id_Prod_Estq, vw_Princ_Prod.Categoria_Prod_Estoq, vw_Princ_Prod.Nome_Prod_Estq, vw_Princ_Prod.Descricao_Prod_Estoq, vw_Princ_Prod.Valor_Venda_Prod_Estoq, vw_Princ_Prod.Foto_Prod_Estoq FROM vw_Princ_Prod INNER JOIN Tb_Categoria ON vw_Princ_Prod.Categoria_Prod_Estoq = Tb_Categoria.Nome_Categoria WHERE (Tb_Categoria.Id_Categoria = @Param1) AND (vw_Princ_Prod.Qtd_Prod_Estoq &gt; 0) AND (vw_Princ_Prod.Tipo_Venda &lt;&gt; 2) ORDER BY NEWID()">
            <SelectParameters>
                <asp:QueryStringParameter DefaultValue="" Name="Param1" QueryStringField="Id_Categoria" />
            </SelectParameters>
        </asp:SqlDataSource>
                    ...
                    <ItemTemplate>
                          ...
                          <div class="product">
								<a href='<%#Eval ("Id_Prod_Estq" , "Prod_item.aspx?Id_Prod_Estq={0}") %>' title='<%# Eval("Nome_Prod_Estq") %>'>
                                    ...
                                    <asp:ImageButton ID="ImageButton2" runat="server" Height="110px" ImageUrl='<%# Eval("Foto_Prod_Estoq") %>' PostBackUrl= '<%#Eval ("Id_Prod_Estq" , "~/Prod_item.aspx?Id_Prod_Estq={0}") %>' Width="128px" OnClick="ImageButton2_Click"/>
									<span class="number">
