<%@ Page Title="" Language="C#" MasterPageFile="~/Administrativo/Adm.Master" AutoEventWireup="true" CodeBehind="Detalhe_Pre_vendas.aspx.cs" Inherits="WebApplication4.Administrativo.Detalhe_Pre_vendas" %>
...
<asp:Repeater ID="rptProdutos" runat="server" DataSourceID="dsLista_pedido" OnItemDataBound="rptProdutos_ItemDataBound"  >
                ...
                <asp:SqlDataSource ID="dsLista_pedido" runat="server" ConnectionString="<%$ ConnectionStrings:BD_SGBConnectionString %>" SelectCommand="SELECT DISTINCT lp.Cod_prd, lp.Produto, lp.Categoria, lp.Marca, lp.Qtd, lp.Valor, lp.Total FROM Tb_Historico_prevenda AS lp INNER JOIN Tb_Pre_Venda AS p ON lp.Cod_Pedido = p.Id_Pre_Venda WHERE (lp.Cod_Pedido = @Param1)">
                    <SelectParameters>
